ngPAWS facilita un sencillo lenguaje de programación para que puedas generar las respuestas que dará el sistema al jugador. Si programar no es lo tuyo, siempre puedes usar [[el asistente de puzles]], que te permitirá añadir los puzles pero ngPAWS programará por ti (tu solo tienes que decirle que condiciones tienen que cumplirse para que el jugador tenga éxito, y que hacer si lo tiene).

El asistente de puzles es válido para personas que no sepan programar, y para personas que sepan pero prefieran un método más cómodo. Aun así, es posible hacer cosas más complicadas programando, por lo que os recomendamos que le echéis un ojo porque no es tan difícil. Algunos de los capítulos siguientes se dedicarán a aprender a "programar" con ngPAWS. Empezamos por la tabla de respuestas, que es donde se programan las respuestas que se da a las órdenes del jugador:

Para que puedas facilitar respuestas a las órdenes del  jugador, debes añadir entradas a la tabla de respuestas (/PRO 0 si estas mirando el fichero desde un editor de texto).
